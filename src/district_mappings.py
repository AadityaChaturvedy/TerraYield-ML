district_mapping = {
    # Andhra Pradesh
    ("andhra pradesh", "alluri sitharama raju"): "Visakhapatnam",
    ("andhra pradesh", "anantpur"): "Anantapur",
    ("andhra pradesh", "annamayya"): "Y.S.R.",
    ("andhra pradesh", "bapatla"): "Guntur",
    ("andhra pradesh", "eluru"): "West Godavari",
    ("andhra pradesh", "kakinada"): "East Godavari",
    ("andhra pradesh", "konaseema"): "East Godavari",
    ("andhra pradesh", "nandyal"): "Kurnool",
    ("andhra pradesh", "ntr"): "Krishna",
    ("andhra pradesh", "palnadu"): "Guntur",
    ("andhra pradesh", "parvathipuram manyam"): "Vizianagaram",
    ("andhra pradesh", "sri potti sriramulu nellore"): "Nellore",
    ("andhra pradesh", "sri sathya sai"): "Anantapur",
    ("andhra pradesh", "tirupati"): "Chittoor",
    
    # Telangana
    ("telangana", "bhadradri kothagudem"): "Khammam",
    ("telangana", "jagitial"): "Karimnagar",
    ("telangana", "jangoan"): "Warangal",
    ("telangana", "jayashankar"): "Warangal",
    ("telangana", "jogulamba gadwal"): "Mahbubnagar",
    ("telangana", "kamareddy"): "Nizamabad",
    ("telangana", "kumuram bheem asifabad"): "Adilabad",
    ("telangana", "mahabubabad"): "Warangal",
    ("telangana", "mahabubnagar"): "Mahbubnagar",
    ("telangana", "mancherial"): "Adilabad",
    ("telangana", "mulugu"): "Warangal",
    ("telangana", "nagarkurnool"): "Mahbubnagar",
    ("telangana", "narayanpet"): "Mahbubnagar",
    ("telangana", "nirmal"): "Adilabad",
    ("telangana", "peddapalli"): "Karimnagar",
    ("telangana", "rajanna sircilla"): "Karimnagar",
    ("telangana", "sangareddy"): "Medak",
    ("telangana", "siddipet"): "Medak",
    ("telangana", "suryapet"): "Nalgonda",
    ("telangana", "wanaparthy"): "Mahbubnagar",
    ("telangana", "warangal rural"): "Warangal",
    ("telangana", "warangal urban"): "Warangal",
    ("telangana", "yadadri bhuvanagiri"): "Nalgonda",
    
    # Karnataka
    ("karnataka", "bagalkote"): "Bagalkot",
    ("karnataka", "ballari"): "Bellary",
    ("karnataka", "belagavi"): "Belgaum",
    ("karnataka", "bengaluru rural"): "Bangalore Rural",
    ("karnataka", "chikkamagaluru"): "Chikmagalur",
    ("karnataka", "kalaburagi"): "Gulbarga",
    ("karnataka", "mysuru"): "Mysore",
    ("karnataka", "shivamogga"): "Shimoga",
    ("karnataka", "tumakuru"): "Tumkur",
    ("karnataka", "vijayanagar"): "Bellary",
    ("karnataka", "vijayapura"): "Bijapur",
    
    # Chhattisgarh
    ("chhattisgarh", "bametara"): "Bemetara",
    ("chhattisgarh", "dakshin bastar dantewada"): "Dantewada",
    ("chhattisgarh", "janjgir - champa"): "Janjgir-Champa",
    ("chhattisgarh", "gaurela-pendra-marwahi"): "Bilaspur",
    
    # Dadra and Nagar Haveli
    ("dadra and nagar haveli", "dadra & nagar haveli"): "Dadra and Nagar Haveli",
    
    # Gujarat
    ("gujarat", "dohad"): "Dahod",
    
    # Haryana
    ("haryana", "gurugram"): "Gurgaon",
    ("haryana", "charki dadri"): "Bhiwani",
    ("haryana", "nuh"): "Mewat",
    
    # Jammu and Kashmir
    ("jammu and kashmir", "baramula"): "Baramulla",
    ("jammu and kashmir", "punch"): "Poonch",

    # Madhya Pradesh
    ("madhya pradesh", "niwari"): "Tikamgarh",

    # Manipur
    ("manipur", "jiribam"): "Imphal East",
    ("manipur", "kakching"): "Thoubal",
    ("manipur", "kamjong"): "Ukhrul",
    ("manipur", "kangpokpi"): "Senapati",
    ("manipur", "noney"): "Tamenglong",
    ("manipur", "pherzawl"): "Churachandpur",
    ("manipur", "tengnoupal"): "Chandel",

    # Meghalaya
    ("meghalaya", "ribhoi"): "Ri Bhoi",
    ("meghalaya", "eastern west khasi hills"): "West Khasi Hills",

    # Mizoram
    ("mizoram", "lawngtlai"): "Lawangtlai",
    ("mizoram", "hnahthial"): "Lunglei",
    ("mizoram", "khawzawl"): "Champhai",
    ("mizoram", "saitual"): "Aizawl",

    # Nagaland
    ("nagaland", "noklak"): "Tuensang",

    # Punjab
    ("punjab", "sri muktsar sahib"): "Muktsar",
    ("punjab", "malerkotla"): "Sangrur",

    # Tamil Nadu
    ("tamil nadu", "kanchipuram"): "Kancheepuram",
    ("tamil nadu", "nagapattinam"): "Nagappattinam",
    ("tamil nadu", "ranipet"): "Vellore",
    ("tamil nadu", "tenkasi"): "Tirunelveli",
    ("tamil nadu", "tiruppattur"): "Vellore",

    # Uttar Pradesh
    ("uttar pradesh", "bara banki"): "Barabanki",
    ("uttar pradesh", "mahrajganj"): "Maharajganj",
    ("uttar pradesh", "shrawasti"): "Shravasti",
    ("uttar pradesh", "siddharthnagar"): "Siddharth Nagar",
    ("uttar pradesh", "prayagraj"): "Allahabad",

    # Arunachal Pradesh
    ("arunachal pradesh", "kamle"): "Lower Subansiri",
    ("arunachal pradesh", "kra daadi"): "Kurung Kumey",
    ("arunachal pradesh", "lepa rada"): "West Siang",
    ("arunachal pradesh", "lower siang"): "West Siang",
    ("arunachal pradesh", "pakke kessang"): "East Kameng",
    ("arunachal pradesh", "shi yomi"): "West Siang",
    ("arunachal pradesh", "siang"): "West Siang",
    ("arunachal pradesh", "upper dibang valley"): "Dibang Valley",
    
    # Assam (Step 1 additions)
    ("assam", "bajali"): "Barpeta",
    ("assam", "biswanath"): "Sonitpur",
    ("assam", "charaideo"): "Sivasagar",
    ("assam", "hojai"): "Nagaon",
    ("assam", "majuli"): "Jorhat",
    ("assam", "south salmara mancachar"): "Dhubri",
    ("assam", "west karbi anglong"): "Karbi Anglong",

    # Jammu and Kashmir (Step 1 additions)
    ("jammu and kashmir", "leh"): "Leh (Ladakh)",
}
